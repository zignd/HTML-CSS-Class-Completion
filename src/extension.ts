...
async function cache(): Promise<void> {
  try {
        ...
        async uri => {
          try {
            Array.prototype.push.apply(
              definitions,
              await ParseEngineGateway.callParser(uri)
            );
          } catch (error) {
            failedLogs += `${uri.path}\n`;
            failedLogsCount++;
          ...
          notifier.notify(
            'eye',
            'Looking for CSS classes in the workspace... (' + progress + '%)',
            false
          );
        },
        { concurrency: 30 }
    ...
    console.log(
      failedLogsCount,
      'failed attempts to parse. List of the documents:'
    );
...
function enableEmmetSupport(disposables: Disposable[]) {
  ...
  languageModes.forEach(language => {
    emmetDisposables.push(
      provideCompletionItemsGenerator(language, emmetRegex, '', '.')
    );
  });
}
...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  workspace.onDidChangeConfiguration(
    async e => {
      try {
        ...
        if (
          e.affectsConfiguration('html-css-class-completion.enableEmmetSupport')
        ) {
          ...
          isEnabled
            ...
            : disableEmmetSupport(emmetDisposables);
        }
      } catch (err) {
        err = new VError(
          err,
  ...
  context.subscriptions.push(
    commands.registerCommand('html-css-class-completion.cache', async () => {
      if (caching) {
        return;
      }
  ...
  ['typescriptreact', 'javascript', 'javascriptreact'].forEach(extension => {
    ...
    context.subscriptions.push(
      provideCompletionItemsGenerator(extension, /\${tw`([\w- ]*$)/)
    );
    context.subscriptions.push(
      provideCompletionItemsGenerator(extension, /css\(tw`([\w- ]*$)/)
    );
    context.subscriptions.push(
      provideCompletionItemsGenerator(extension, /css={tw`([\w- ]*$)/)
    );
